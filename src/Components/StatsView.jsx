...
import { motion, useMotionValue, useTransform } from "framer-motion"; // Imported motion from framer-motion
  ...
  useEffect(() => {
    const fetchCampaigns = async () => {
      try {
        ...
        const incrementValueCompleted = Math.ceil(completedCampaigns / 100); // Adjust speed by changing divisor
        ...
        for (let i = 0; i <= completedCampaigns; i += incrementValueCompleted) {
          await new Promise((resolve) => setTimeout(resolve, 60)); // Adjust timeout for speed
